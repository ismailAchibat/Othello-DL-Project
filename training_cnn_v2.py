...
from networks_2507454 import CNN_v2  # Changed import
...
def main(args):
    ...
    conf["path_save"] = f"save_models_CNN_v2"
    conf['epoch'] = args.num_epochs
    conf["earlyStopping"] = 20
    conf["len_inpout_seq"] = len_samples

    model = CNN_v2(conf).to(device)  # Changed model instantiation
    
    if args.optimizer.lower() == 'adam':
        opt = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay) # Added weight_decay
    elif args.optimizer.lower() == 'sgd':
        opt = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=args.weight_decay) # Added weight_decay
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train a regularized CNN model (v2) for Othello.')
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-4, help='Weight decay (L2 regularization)') # Added argument
